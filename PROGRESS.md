...
**Runnable augmentation started (2026-07-05, Prompt 4):** Added the strategy to `CLAUDE_CONTENT_PROMPTS.md` (Prompt 4) and shipped the first two runnable non-DSA problems in `prisma/seed-data/runnable-systems.ts`, both verified end-to-end with `runLocalTests` (correct solution passes all, buggy starter fails): `async-worker-at-least-once` (asyncio, at-least-once retry invariant → queues-workers) and `keyset-pagination-consistency` (in-memory sqlite3, no-duplicate/no-skip invariant → cursor-pagination). The existing local runner already awaits coroutines and runs `sqlite3`/`threading`, so no runner change was needed. Augmentation audit of all 231 problems: **70 already runnable**, **~55 can be made runnable now** (invariant-testable debugging/optimization/databases/concurrency — ~18 already ship starterCode), **10 need a sql/plan-cost oracle** (N+1, index-usage, offset-scan cost — output-invisible, needs a `sql`/`scenario` harness), **19 are C++** (need a compiled sandbox or stay read/reason), **77 stay written-answer** (open design/system-design/analysis).

**Runnable augmentation batch 2 (2026-07-05, Prompt 4):** Added four more runnable siblings to `runnable-systems.ts` (now 6 total; seed bank 235 problems), each verified end-to-end with `runLocalTests` (buggy starter fails the distinguishing cases, full solution passes all). Two use a correctness-distinguishing bug, one uses an ordering bug, and one uses a **call-budget stub** — the technique that makes an *optimization* (not just a correctness) property testable: the fake dependency raises when called too many times, so the slow per-item version fails and the batched version passes. New problems: `idempotent-charge-on-retry` (payments idempotency key, dedup-by-key → database-deadlocks-idempotency), `dedup-redelivered-emails` (at-least-once redelivery, idempotent worker → queues-workers), `cache-invalidate-after-commit` (commit-before-cache ordering, no phantom reads → caching-strategies), and `batch-merchant-enrichment` (N+1 → dedupe+batch under a call budget → n-plus-one-queries). Reusable pattern captured: for optimization problems where the starter is *correct but slow*, encode the efficiency target as a budgeted stub dependency that raises on overuse, so the test distinguishes fast from slow without a real profiler.

**Runnable augmentation batch 3 — budgeted-stub optimizations (2026-07-05, Prompt 4):** Added three more runnable siblings to `runnable-systems.ts` (now 9 total; seed bank 238 problems), applying the budgeted-stub technique to *optimization* problems whose starter is correct-but-slow, each verified end-to-end with `runLocalTests` (starter 0/3, 0/3, 0/2; full solution passes all): `orm-n-plus-one-query-count` (N+1 dashboard under a **query-count budget** — the Database raises past N queries, so per-team/per-member fetching fails and batched fetching passes → n-plus-one-queries), `settlement-matcher-linear` (quadratic matcher under a **scan-count budget** — ConfirmBook.all() charges one unit per row with a one-full-scan budget, so re-scanning per trade fails and build-index-once passes, preserving earliest-first duplicate semantics → arrays-hashmaps-two-pointers), and `feed-payload-normalization` (serialize-less as a **checkable output shape** — the normalized {items with ids, users side-map} shape is a correctness test the inlined-nested-user starter fails → http-request-lifecycle). Confirms the budgeted-stub generalizes across call-count, scan-count, and shape families; documented in Prompt 4.

**Runnable augmentation batch 4 — transactions/atomicity + backpressure (2026-07-05, Prompt 4):** Added three more runnable siblings to `runnable-systems.ts` (now 12 total; seed bank 241 problems), making the previously-empty transactions-isolation and reliability-backpressure modules runnable. All verified end-to-end with `runLocalTests` (starter 1/4, 1/4, 2/4; full solution passes all): `inventory-oversell-conditional-update` (sqlite3; the guarded-UPDATE repair from the lost-update lesson — starter validates every order against a stale up-front read and oversells, fix uses `UPDATE ... WHERE qty >= ?` + rowcount → transactions-isolation), `account-transfer-atomicity` (sqlite3; validate-then-apply all-or-nothing — starter debits before checking the destination exists and vanishes money, fix validates both accounts + funds first → transactions-isolation/database-deadlocks-idempotency), and `bounded-queue-load-shedding` (a `BoundedInbox` that raises `QueueFull` at capacity — starter enqueues blindly and crashes on overflow, fix sheds with a counter → reliability-backpressure). Key technique note: the classic lost-update/race bugs were made **deterministic without threads** by reproducing the staleness structurally (validate against a snapshot read once up front) rather than relying on a flaky interleaving — the same faithful-vs-testable move as the async-worker sibling.

**SQL plan-cost oracle (2026-07-05, Prompt 4 infra):** Built the previously-blocked plan-cost harness so index-selection problems become gradable — their output is identical whether the query is fast or slow, so a value check cannot grade them. New `src/lib/sql-plan-runner.ts` (`runSqlPlanTests`) instruments an in-memory SQLite db: it applies the candidate's SQL (a `CREATE INDEX`), captures `EXPLAIN QUERY PLAN` of a fixed query, and asserts on plan SHAPE — index usage vs full `SCAN`, `USING COVERING INDEX` (index-only), and whether an `ORDER BY` still needs `USE TEMP B-TREE`. SQLite picks its plan from schema+indexes (no ANALYZE), so signals are deterministic and row-count-independent. Wired end to end: `testHarnessType: "sql_plan"` and a `"sql"` language through `schemas.ts`, the run API route (`/api/submissions/run` branches to the SQL oracle), and the workspace UI (SQL editor + label). Unit-tested in `src/lib/sql-plan-runner.test.ts` (5 tests: no-index full scan fails, single-column index rejected for still needing a temp sort, composite passes, covering detected). Three authored, plan-graded problems in `prisma/seed-data/sql-plan.ts` (seed bank 244), each verified starter-fails/solution-passes: `index-the-login-lookup` (single-column index → sql-indexes), `composite-index-filter-and-order` (equality-then-sort composite, no temp B-tree → composite-indexes), `covering-index-avoids-table-lookup` (index-only read → composite-indexes). Closes the "needsSqlOracle" bucket from the audit; the remaining infra gap is the 19 C++ problems (compiled sandbox).
