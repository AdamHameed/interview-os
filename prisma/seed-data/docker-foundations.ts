...
export const dockerFoundationProblems = defineProblems([
...

  {
    ...
    prompt:
      "Read this shell session captured on a Linux host running two containers, then judge the statements below.\n\n```console\n$ uname -r                       # host kernel version\n5.15.0-91-generic\n$ docker run -d --name web nginx           # container 1\n$ docker run -d --name api python:3.12      # container 2\n$ docker exec web uname -r        # kernel seen INSIDE the container\n5.15.0-91-generic                # <-- identical to the host: no container kernel\n$ ps -e -o pid,comm | grep nginx  # the containerized process, seen from the HOST\n  4812 nginx                      # it is just a host process in isolated namespaces\n$ cat /sys/fs/cgroup/cpu.max      # container 1's cgroup CPU limit\nmax 100000                        # 'max' = UNLIMITED unless you set --cpus\n```\n\nUsing the session and concrete mechanisms (namespaces, cgroups, hypervisor, kernel), answer each statement **True**, **False**, or **It depends** with a one–two sentence justification.\n\n1. The two containers share the host's kernel.\n2. A container provides stronger isolation than a VM because it uses fewer resources.\n3. Running a process in a Docker container prevents it from consuming all CPU on the host by default (see the `cpu.max` output).\n4. This same Ubuntu-based image can run directly on a macOS host without any virtualization layer.\n5. A VM and a container started from the same app can both listen on port 8080 without conflict — explain how each achieves port isolation.",
    constraints:
