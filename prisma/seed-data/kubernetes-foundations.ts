...
export const kubernetesFoundationProblems = defineProblems([
...

  {
    ...
    confidenceLevel: "warmup",
    prompt:
      "Each manifest below has its `kind:` blanked out. Read the distinguishing spec fields and fill in the correct Kubernetes workload type from: **Pod, Deployment, StatefulSet, Job, CronJob**. Justify each from the fields that force the choice.\n\n```yaml\n# 1 —\nkind: ____\nspec:\n  replicas: 3                       # stateless REST API, must survive node failures\n  selector: { matchLabels: { app: api } }\n  template: { spec: { containers: [{ name: api, image: api:v2 }] } }\n---\n# 2 —\nkind: ____\nspec:\n  serviceName: postgres             # stable DNS: postgres-0.postgres\n  replicas: 3\n  volumeClaimTemplates:             # each replica keeps its OWN persistent volume\n    - metadata: { name: data }\n      spec: { accessModes: [ReadWriteOnce], resources: { requests: { storage: 20Gi } } }\n---\n# 3 —\nkind: ____\nspec:\n  completions: 1                    # run to completion exactly once, on deploy\n  backoffLimit: 3\n  template: { spec: { restartPolicy: Never, containers: [{ name: migrate, image: migrator:v2 }] } }\n---\n# 4 —\nkind: ____\nspec:\n  schedule: \"0 2 * * *\"             # 02:00 UTC every day\n  jobTemplate: { spec: { template: { spec: { restartPolicy: OnFailure, containers: [{ name: report, image: reporter }] } } } }\n---\n# 5 — created ad hoc:  kubectl run debug --rm -it --image=busybox -- sh\n#     (one throwaway instance you attach to and delete)\nkind: ____\n```",
    constraints:
...

  {
    ...
    confidenceLevel: "warmup",
    prompt:
      "Each Service manifest below has its `type:` (or the Headless marker) blanked out, followed by the requirement it must satisfy. Read the spec and requirement, then fill in the correct Service type from: **ClusterIP, NodePort, LoadBalancer, Headless (clusterIP: None)**. For each, name one property that makes your choice correct and one that makes another option wrong.\n\n```yaml\n# 1 — internal payments microservice, reachable ONLY by other pods in the cluster\napiVersion: v1\nkind: Service\nmetadata: { name: payments }\nspec:\n  type: ____\n  selector: { app: payments }\n  ports: [{ port: 8080, targetPort: 8080 }]\n---\n# 2 — public web app on a cloud provider (GKE/EKS/AKS), needs an external DNS-able address\nkind: Service\nmetadata: { name: web }\nspec:\n  type: ____                 # provisions an external cloud load balancer + external IP\n  selector: { app: web }\n  ports: [{ port: 80, targetPort: 8080 }]\n---\n# 3 — Kafka StatefulSet: each broker needs its OWN stable DNS name (kafka-0.kafka-svc ...)\nkind: Service\nmetadata: { name: kafka-svc }\nspec:\n  clusterIP: ____            # what value makes this Headless, returning per-pod DNS A records?\n  selector: { app: kafka }\n  ports: [{ port: 9092 }]\n---\n# 4 — dev/test: expose on every node's IP for laptop access, no cloud load balancer\nkind: Service\nmetadata: { name: devtest }\nspec:\n  type: ____                 # opens the same port on every node (e.g. 30080)\n  selector: { app: devtest }\n  ports: [{ port: 80, targetPort: 8080 }]\n```",
    constraints:
