...
export const httpQueuesFoundationProblems = defineProblems([
...

  {
    ...
    confidenceLevel: "core",
    prompt:
      "A services team is adding automatic retry logic to an HTTP client library. For each request below, state: (a) whether it is safe to retry without coordination, (b) the HTTP property that determines this (safe, idempotent, or neither), and (c) the worst-case outcome if the server has already processed the original request but the client did not receive the response.\n\n1. `GET /users/42` — fetch user profile\n2. `DELETE /sessions/abc123` — log out a session\n3. `PUT /users/42` with a full user object — replace user record\n4. `POST /payments` — charge $100 to a card\n5. `POST /payments` with `Idempotency-Key: req-9f2k-a3b1` header — charge $100 with client-supplied key\n6. `PATCH /counters/page-views` where the server adds 1 to the count",
    constraints:
...

  {
    ...
    prompt:
      "For each queue configuration below, classify the delivery guarantee as **at-most-once**, **at-least-once**, or **effectively-exactly-once** (at-least-once delivery + idempotent consumer), and explain the mechanism that produces the guarantee.\n\n**Config A — fire-and-forget UDP notification:**\nA service sends a UDP datagram to a log aggregator on best-effort: no ACK, no retry, no persistence.\n\n**Config B — SQS standard queue, visibility timeout 30 s:**\nA worker calls `receive_message`, processes the job, and calls `delete_message` only after successful processing. If the worker crashes before deleting, the message becomes visible again after 30 s.\n\n**Config C — SQS + idempotency key in worker:**\nSame as Config B, but the worker checks a database table for the message ID before processing; if found, it skips processing and calls `delete_message`.\n\n**Config D — Kafka consumer, auto-commit offsets enabled, commit interval 5 s:**\nKafka automatically commits the consumer's offset every 5 seconds. If the consumer crashes at second 4 after processing a batch, it resumes from the last committed offset.",
    constraints:
